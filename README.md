...
| select (config) | `start_mode` | `user_settings.start_mode` (PUT via `set_user_settings`, key `startMode`) — options come from the charger's reported `allowedValues`; see [Known limitations](#known-limitations) |
| select (config) | `cable_settings` | `user_settings.cable_settings` (PUT via `set_user_settings`, key `cableSettings`) — options come from the charger's reported `allowedValues`; see [Known limitations](#known-limitations) |
...
`start_mode` carries a separate, confirmed cloud defect that will make it look broken — see [Known limitations](#known-limitations) before you automate it. `cable_settings` writes normally but what its options *do* is uncharacterised.
...
- **A `start_mode` write is invisible until another setting is written — a Ratio cloud defect.** The write is accepted and persists correctly, but no subsequent `GET` reflects it until some other `userSettings` key is written, at which point the new value appears. Confirmed by measurement: 60 s of forced refreshes still returned the old value, and a single write to `cable_settings` revealed it immediately. **The vendor's own app has the same defect** — changing start mode there, force-closing and reopening shows the old value too, and that write was likewise still pending, revealed by the same flush. So `select.<charger>_start_mode` will report success and then display the old value indefinitely. Nothing in this integration can fix that: there is no read that returns the truth, so no amount of refresh tuning helps. Your change *has* taken effect. The integration deliberately does not fake the value, so that a write the cloud genuinely rejects stays visible as a failure. See [#80](https://github.com/aaearon/home-assistant-ratio/issues/80).
- **`start_mode` and `cable_settings` options are exposed, not characterised.** Both selects offer exactly the values the charger reports in `allowedValues` and nothing more — there is no fallback list, so if the cloud omits `allowedValues` the select has no options. What the options actually *do* has not been verified: for `cable_settings`, the difference between `LockWhenCarConnected`, `LockAutomatically` and `LockAlways` is uncharacterised, and `LockAlways` in particular has not been observed in effect — it is offered because the charger says it is allowed, not because we know it keeps the cable locked. Likewise `start_mode`'s `Manual`: writing it was confirmed to persist, but the behavioural difference from `Auto` (and how it interacts with the `charging` switch and `start_charge`) has not been established. Treat both as write-and-observe.
...
- **`import_session_history` only accepts a backfill into an empty statistic series.** The service raises `ServiceValidationError` if `begin_time` predates the history-import baseline (latest imported session timestamp, advanced by the live polling coordinator) for any charger, *or* if that charger already has any external energy statistics recorded at all — regardless of where the requested window falls relative to them. The recorder overwrites rows by `(metadata_id, start)` without recalculating later rows, so there is no safe way to insert into a series that already has data; manual backfill is only for charging history that predates the integration's first successful poll. **Re-adding the integration does not reset this.** The statistic id (`ratio:energy_<serial>`) is derived from the charger serial number, not the config entry, so the recorder series survives a remove-and-re-add. The live polling coordinator reads that series back on its first poll after a re-add: it resumes the running total from the recorder's last row instead of restarting at 0, starts its fetch window at the hour after that row instead of re-running the 30-day backfill, and discards any session that would land in an hour the recorder has already written.
